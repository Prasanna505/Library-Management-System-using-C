...
struct IS
{
    char name[200], BookName[200], USN[15];
    int BookId;
    struct Date doi;
    struct Date dor;
    struct IS *next;
    ...
    do{
           ...
           if(fptr==NULL)
            ...
            else
            {
                while(temp!=NULL)
                {
                    fprintf(fptr,"\n\n%s\n%s\n%s\n%d\n%d/%d/%d", temp->name,temp->USN,temp->BookName,temp->BookId,temp->doi.dd, temp->doi.mm, temp->doi.yyyy);
                    temp=temp->next;
                }
            }
            printf("Continue(1/0)?\n");
...
int xsearch(char name[],struct IS *p)
{
    ...
    if(!strcmp(name,p->name))
    {
            ...
            fprintf(fp,"\n\n%s\n%s\n%s\n%d\n%d/%d/%d\n%d/%d/%d", p->name, p->USN, p->BookName, p->BookId, p->doi.mm, p->doi.dd, p->doi.yyyy,r->dor.mm,r->dor.dd,r->dor.yyyy);
    {
...
void Return_book()
{
    ...
    if((fptr = fopen("IssueList.txt","r")) != NULL)
    {
        ...
        while(!feof(fptr))
            {
                fscanf(fptr,"\n\n%s\n%s\n%s\n%d\n%d/%d/%d", p->name,p->USN,p->BookName,&p->BookId,&p->doi.mm, &p->doi.dd, &p->doi.yyyy);
                xsearch(name,p);
            }
            fclose(fptr);
    }
...
{
    char name[200],BookName[200],USN[15],c;
    int BookId,date1,mon1,year1;
    FILE *fptr;
    fptr=fopen("IssueList.txt","r");
    if (fptr == NULL)
    {
  ...
  else if (fptr!= NULL)
    {
        if(c=fgetc(fptr)!= EOF)
        {
            while(!feof(fptr))
            {
                fscanf(fptr,"\n\n%s\n%s\n%s\n%d\n%d/%d/%d",name,USN,BookName,&BookId,&mon1,&date1,&year1);
                printf("\n\nName:%s\nUSN:%s\nBookName:%s\nBookID:%d\nDateofIssue:%d/%d/%d",name,USN,BookName,BookId,mon1,date1,year1);
            }
...
{
    char name[200],BookName[200],USN[15],z;
    ...
    else if(fptr!=NULL)
    {
        if(z=fgetc(fptr)!= EOF)
        {
            while(!feof(fptr))
            {
                fscanf(fptr,"\n\n%s\n%s\n%s\n%d\n%d/%d/%d\n%d/%d/%d",name,USN,BookName,&BookId,&mon1,&date1,&year1,&mon2,&date2,&year2);
